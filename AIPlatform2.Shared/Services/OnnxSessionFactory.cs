// Example: OnnxSessionFactory.cs
// Requires: using Microsoft.ML.OnnxRuntime;
// Optional provider packages: Microsoft.ML.OnnxRuntime.Gpu, Microsoft.ML.OnnxRuntime.DirectML, Microsoft.ML.OnnxRuntime.CoreML
// Add whichever provider NuGets you will ship / instruct users to install.
    ...
    public static class OnnxSessionFactory
{
    ...
    {
        "CUDA",
        "TensorRT",    // optional, only if you ship TensorRT provider
        "DirectML",
        "CoreML",
        "CPU"
    };
    ...
    public static InferenceSession CreateSession(string modelPath, SessionOptions baseOptions = null)
    {
        ...
        foreach (var p in ProviderPriority)
        {
            try
            {
                ...
                {
                    // Optional - uncomment if you ship TensorRT EP and have the method available.
                    // options.AppendExecutionProvider_TensorRT();
                    // triedProviders.Add("TensorRT");
                }
                else if (p == "DirectML")
                {
